...
# Create a clean prompt file (no empty lines) in temp
CLEAN_PROMPTS=$(mktemp /tmp/clean_prompts_XXXXXX.txt)
...
for i in "${!GPU_ARRAY[@]}"; do
    ...
    # Create temp prompt file for this GPU
    TEMP_FILE=$(mktemp /tmp/prompts_gpu${GPU}_XXXXXX.txt)
...
# Wait for all processes, track failures
FAIL=0
for pid in "${PIDS[@]}"; do
    wait "$pid" || FAIL=1
done
