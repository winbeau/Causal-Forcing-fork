...
def headkv_attention(
    ...
    def run_varlen(
        ...
        return out_chunk.squeeze(1).reshape(b, h, lq_chunk, d).transpose(1, 2)

    # Decoupled path: per-frame varlen with sink grid decoupling
    use_decoupled = (
    ...
    if use_decoupled:
        ...
        for offset in range(0, lq, frame_seqlen):
            ...
            if hasattr(kv_cache, "get_decoupled_flat_kv_and_frames"):
                k_flat, v_flat, cu_seqlens_k, max_seqlen_k, _k_frame_ids = kv_cache.get_decoupled_flat_kv_and_frames(
    ...
    # Flat path: post-prune RoPE or pre-RoPE
