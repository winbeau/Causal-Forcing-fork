...
def inspect_headkv_csv_shape(csv_path: str) -> tuple[int, int, bool]:
    with open(csv_path, "r", encoding="utf-8") as handle:
        rows = [row for row in csv.reader(handle) if row]
    if not rows:
        return 0, 0, False
    return len(rows), max(len(row) for row in rows), any(len(row) > 3 for row in rows)


def validate_headkv_matrix_csv_shape(
    csv_path: str,
    num_layers: int,
    num_heads: int,
    *,
    label: str = "policy",
    runtime_label: str = "30x12 / Wan2.1-T2V-1.3B",
) -> None:
    if not csv_path or not os.path.exists(csv_path):
        return

    row_count, max_cols, _ = inspect_headkv_csv_shape(csv_path)
    if row_count == 0:
        raise ValueError(
            f"HeadKV {label} CSV is empty: {csv_path}. "
            f"Expected a {num_layers}x{num_heads} matrix for the current {runtime_label} inference path."
        )

    with open(csv_path, "r", encoding="utf-8") as handle:
        rows = [row for row in csv.reader(handle) if row]
    all_triplets = all(len(row) == 3 for row in rows)
    is_matrix = (not all_triplets) or (max_cols == num_heads)
    if not is_matrix:
        return

    if row_count != num_layers or any(len(row) != num_heads for row in rows):
        raise ValueError(
            "HeadKV policy/model shape mismatch: "
            f"{label} CSV {csv_path} has shape {row_count}x{max_cols}, "
            f"but the loaded model expects {num_layers}x{num_heads}. "
            f"This port currently targets the {runtime_label} inference path."
        )
